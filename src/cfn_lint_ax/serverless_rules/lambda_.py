...
class LambdaTracingRule(CloudFormationLintRule):
    ...
    def match(self, cfn):
        ...
        for key, value in cfn.get_resources(["AWS::Lambda::Function"]).items():
            tracing_mode = (
                value.get("Properties", {}).get("TracingConfig", {}).get("Mode", None)
            )
...
class LambdaStarPermissionRule(CloudFormationLintRule):
    ...
    source_url = (
        "https://awslabs.github.io/serverless-rules/rules/lambda/star_permissions/"
    )
    tags = ["lambda"]
    ...
    def _get_principals(self, properties) -> List[str]:
        ...
        for statement in properties.get("AssumeRolePolicyDocument", {}).get(
            "Statement", []
        ):
    ...
    def _get_valid_functions(self, log_groups):
        ...
        for resource in log_groups.values():
            log_group_name = Value(
                resource.get("Properties", {}).get("LogGroupName", None)
            )
    ...
    def match(self, cfn):
        ...
        for function_ref, function in functions.items():
            ...
            if not log_group_found:
                matches.append(
                    RuleMatch(
                        ["Resources", function_ref], self._message.format(function_ref)
                    )
                )
...
class LambdaAsyncNoDestinationRule(CloudFormationLintRule):
    ...
    def match(self, cfn):
        ...
        for value in event_invoke_configs.values():
            ...
            if (
                function_name in async_functions.values()
                and on_failure_destination is not None
            ):
